...
const imgloader =
  ...
  (record: FileSchema | undefined): Promise<void> => {
...
const promiseFind = (arr: FileSchema[], type: 'IMAGE' | 'VIDEO') => {
  let done = false;
  return new Promise<FileSchema | undefined>((resolve, reject) => {
    const queueNext = async (record: FileSchema | undefined) => {
      if (isUndefined(record)) {
        return undefined;
      }
...
// one loading
export const MediaResource = (module: FileSchema, type: 'IMAGE' | 'VIDEO') => {
